...
Check out the [repository](https://github.com/thliebig/openEMS-Project) to get a better look at what it can do, and leave a star for the repository to support the amazing work of Thorsten Liebig and the other contributors.
...
### Octave
#### Unix-likes
Go to the [Octave Wiki](https://wiki.octave.org/Category:Installation)
...
```
#### Windows
On the [download page](https://octave.org/download) get the version that fits your system
...
## The antenna
The antenna is an axial-mode RHCP *(right-hand circular polarisation)* helix antenna for a frequency of $f=869.525\,\mathrm{MHz}$. Find the specifications in the [Design Overview](/docs/design_overview.md).
...
The antenna is not simulated at one frequency. The excitation is a short pulse, which gives us the whole band in a single run, like a VNA sweep.
...
excited frequency, not at `f0`, but at `c0/(f0+fc)/30` = 309 mm / 30 → 10 mm. The ramp has to be
gradual and not an abrupt size jump which reflects waves off the grid itself.
...
The parametric equation of a coil, sampled into points and given a thickness:
...
![All three result panels in one figure](/docs/images/helical_antenna_simulation_images/00_overview.png)
...
![Real and imaginary part of the input impedance over the excited band](/docs/images/helical_antenna_simulation_images/01_impedance.png)

![S11 referenced to 50 ohm and to 140 ohm](/docs/images/helical_antenna_simulation_images/02_s11.png)
...
![Directivity vs theta, total and split into RHCP and LHCP](/docs/images/helical_antenna_simulation_images/03_polarization.png)
