...
This is REV C — hardened after a 4-lens adversarial DFM/RF/sealing/process review (rev B) and then
fixed against a real slicer pass (rev C). The RF geometry (10 mm patch->window air gap, ~0.115*lambda0
wall-to-patch clearance, mount metal shielded behind the ground plane) PASSED throughout; the fixes
are all manufacturing/sealing/process:
  * REV C (slicer findings): the mount ears now EMBED 4 mm into the wall so they FUSE with the body
    (rev B's 0.01 mm touch sliced as 3 detached islands = "no consistent groundplane"); the breather
    vent was moved to (50,50), clear of the standoff bosses it overlapped (now assert-guarded); and
    WALL 6->4 mm so the wall prints ALL-PERIMETER solid (watertight) instead of with a porous infilled
    core — the gasket ribs (1.25 mm) + the window LIP bond carry the seal, no thick flat land needed.
  * WALL gasket: the foam-gasket groove leaves a >=1.2 mm rib each side (rev A left 0.5 mm ribs —
    too thin to slice, seal, or bond).
  * FLOOR 3->4 mm (>=6 fused bottom layers) for a watertight base.
  * Coax bore is a TEARDROP with a FLAT BOTTOM held >=3 mm above the floor (rev A left a 0.8 mm web
    = guaranteed bottom-of-wall leak + an unprintable 12 mm crown overhang). + zip-tie strain relief.
  ...
  * Standoffs take M3 BRASS HEAT-SET INSERTS (pilot 3.4) + NYLON M3 screws (rev A self-tap strips in
    nylon). Insert sits below the GP (shielded); the screw is nylon -> RF-safe even on the patch side.
  ...
  * Inner floor/wall + boss-base fillets relieve ABS cooling stress; ears trimmed to fit a 220 bed
    with brim room; build-time asserts enforce every DFM/seal rule so a future param edit cannot
    silently re-introduce a thin web/rib/boss or an over-bed footprint.
...
def _params_block() -> str:
    return f"""// ===== AUTO-GENERATED by casing/enclosure.py (REV B) — DO NOT hand-edit; change the Python =====
...
// four board standoff bosses (M3 brass heat-set insert; nylon M3 screw from the patch side)
module standoffs() {
    for (sx = [-1, 1], sy = [-1, 1])
        translate([sx * hole_xy, sy * hole_xy, floor_th])
            difference() {
                ...
                translate([0, 0, -0.1]) cylinder(d = so_pilot, h = standoff_h + 0.2);  // insert bore
            }
}
...
// (a 0.01 mm touch sliced as 3 separate islands -> the "no consistent groundplane" bug).
module ears() {
...
def main():
    ap = argparse.ArgumentParser(description="Patch-antenna enclosure (Python -> OpenSCAD), rev B.")
    ...
    print(f"  Standoffs         : 4 x M3 BRASS heat-set @ (+/-{HOLE_XY:.0f}, +/-{HOLE_XY:.0f}), boss wall {BOSS_WALL:.1f} mm + base fillet")
    ...
    print("  2. Mount board on standoffs (brass insert + NYLON M3). Route coax, connect TEMPORARILY.")
